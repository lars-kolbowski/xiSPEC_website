    ...
    $(function() {
		...
		window.modTable = $('#modificationTable').DataTable( {
				...
				},
			{
				"render": function ( data, type, row, meta ) {
					data = 0;
					for (var i = 0; i < window.SpectrumModel.userModifications.length; i++) {
						if(window.SpectrumModel.userModifications[i].id == row.id){
							data = window.SpectrumModel.userModifications[i].mass;
							var found = true;
						}
					}
					if (!found){
						for (var i = 0; i < window.SpectrumModel.knownModifications['modifications'].length; i++) {
							if(window.SpectrumModel.knownModifications['modifications'][i].id == row.id)
								data = window.SpectrumModel.knownModifications['modifications'][i].mass;
						}
					}
			...
			},
			{
				"render": function ( data, type, row, meta ) {
					for (var i = 0; i < window.SpectrumModel.userModifications.length; i++) {
						if(window.SpectrumModel.userModifications[i].id == row.id){
							data = window.SpectrumModel.userModifications[i].aminoAcids.join("");
							var found = true;
						}
					}
					if (!found){				
						for (var i = 0; i < window.SpectrumModel.knownModifications['modifications'].length; i++) {
							if(window.SpectrumModel.knownModifications['modifications'][i].id == row.id){						
								data = data.split(",");
								data = _.union(data, window.SpectrumModel.knownModifications['modifications'][i].aminoAcids);
								data.sort();
								data = data.join("");
		...
		window.SettingsPepInputView = new PepInputView({model: SpectrumModel, el:"#settingsPeptide"});
        ...
        //SettingsSpectrumModel.set({JSONdata: json_req});
            ...
            <div class="mainContent">
                <div id="spectrumPanel">
                	<div id="spectrumControls">
                		...
                		<label id="colorSelectorLabel" class="btn">Colour scheme:</label>
                		...
                		<form id="setrange">
                			...
                			<label for="lockZoom" title="Lock current zoom level" id="lock" class="btn">🔓</label>
                			<input type="submit" id="rangeSubmit" value="Set" class="btn btn-1 btn-1a">
                			<span id="range-error"></span>
                			<button id="reset" title="reset to initial zoom level" class="btn btn-1 btn-1a">All</button>
...
<!-- 					<div id="settings" class="dynDiv_moveDiv">
					 <p style="padding: 10px;"> This is a test div with ByRei DynDiv, resize it ;) </p>
					 <div class="dynDiv_moveParentDiv" style="cursor: move;"><span class="dynTitle">Settings</span><i class="fa fa-times-circle closeButton"></i></div>
					...
					<div class="dynDiv" id="settingsWrapper" style="display: none; z-index: 2; right: 5%; top: 10%; width: 800px; height: 600px;">
						...
						<i class="fa fa-times-circle closeButton" id="closeSettings"></i></div>
						...
						<div id="settings_main">
							<form id="settingsForm" method="post">
								...
								<div style="width:68%;display:inline;">
									<label class='input-w' for="settingsCL"><span class="label">Cross-linker mod mass: </span>
										<input class="form-control" style="margin-right:2%;" required="" id="settingsCL" placeholder="CL mod mass" name="clModMass" autocomplete="off">
									</label>

									<label class='input-w' for="settingsPrecursorZ"><span class="label">Precursor charge: </span>
						  				<input class="form-control" style="margin-right:2%;" required="" id="settingsPrecursorZ" type="number" min="1" placeholder="Charge" name="preCharge" autocomplete="off">
									</label>

									<label class='input-w' for="settingsFragmentation"><span class="label">Fragmentation method: </span>
										<select class="form-control" style="margin-right:2%;width:10%;display:inline;" id="settingsFragmentation" name="fragMethod">
									...
									<label class='input-w' for="settingsTolerance"><span class="label">MS2 tolerance: </span>
								...
								<div style="margin-top:30px; text-align: center">
									...
									<input class="btn btn-1 btn-1a network-control" type="button" value="Cancel" id="settingsCancel">
								</div>
							</form>
						</div>
					</div>
